...
-- All tables mirror InstaPlate recipe JSON fields exactly

-- Recipes table - main recipe storage
...
-- Recipe images for additional photos
...
-- Packs for grouping recipes for export
...
-- Junction table for pack-recipe relationships with ordering
...
-- Usage tracking for "Used Where" feature
CREATE TABLE IF NOT EXISTS usage_events (
    id TEXT PRIMARY KEY,
    recipe_id TEXT NOT NULL,
    type TEXT NOT NULL, -- exported_pack, featured, collection, campaign, other
    ref_id TEXT,
    where_text TEXT,
    created_at TEXT NOT NULL,
    FOREIGN KEY (recipe_id) REFERENCES recipes(id) ON DELETE CASCADE
